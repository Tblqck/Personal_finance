...
CATEGORIES = [
    "food", "rent", "bills", "shopping", "transport",
...
def predict_category(text):
    """Predict expense category using AI."""
    cat_prompt = f"""
Classify this expense into ONE simple category word or phrase from:
{', '.join(CATEGORIES)}.
...
# HANDLE EXPENSE
# ----------------------
def handle_expense(user_id, message, user_data):
    base_user_id = str(user_id).strip()
    retry_tracker = load_json(RETRY_TRACKER_FILE)
    retry_tracker.setdefault(base_user_id, 0)

    # Initialize user structure safely
    user_data.setdefault(base_user_id, {
        "sections": {},
        "expenses": [],
        "session_info": {}
    })
    user_data[base_user_id].setdefault("sections", {})
    user_data[base_user_id].setdefault("expenses", [])
    user_data[base_user_id].setdefault("session_info", {})

    # Activate expense section and ensure pending placeholder
    user_data[base_user_id]["sections"]["add_expense"] = "on"
    user_data[base_user_id]["session_info"].setdefault(
        "pending", {"amount": None, "currency": None, "category": None}
    )
    pending = user_data[base_user_id]["session_info"]["pending"]

    # 1️⃣ Update pending fields if new info is provided
    ...
    # Save updated placeholder immediately
    user_data[base_user_id]["session_info"]["pending"] = pending
    save_json(USER_DATA_FILE, user_data)

    # 2️⃣ If any info is missing, ask user and keep placeholder
    if not all(pending.values()):
        missing = [k for k, v in pending.items() if not v]
        return {"message": f"🤔 I need more information: {', '.join(missing)}.", "hash": None}

    # 3️⃣ Generate transaction hash & timestamp
    timestamp = datetime.now().isoformat()
    hash_input = f"{base_user_id}{pending['amount']}{pending['currency']}{pending['category']}{timestamp}"
    record_hash = f"exp{base_user_id}{hashlib.md5(hash_input.encode()).hexdigest()}"

    # 4️⃣ AI-generated comment
    comment_prompt = f"""
Write a short human-like remark about this expense transaction:
...
"""
    ...
    if err or not transaction_comment:
        ...
        if retry_tracker[base_user_id] >= 4:
            return {"message": "⚠️ The chat system is down. Please contact admin on WhatsApp (+234 8169482136).", "hash": None}
        transaction_comment = "Expense recorded successfully (AI summary unavailable)."
    ...
    # 5️⃣ Save transaction with deduplication
    budget = load_json(BUDGET_JSON)
    budget.setdefault("transactions", [])
    budget["transactions"] = [t for t in budget["transactions"] if t["id"] != record_hash]
    ...
    # 6️⃣ Reset session after full transaction is saved
    user_data[base_user_id]["sections"]["add_expense"] = "off"
